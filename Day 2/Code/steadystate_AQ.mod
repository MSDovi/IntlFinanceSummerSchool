...
Omega=log(beta*(1-omega+omega*exp(kappa)));
Thetax=log(theta*(1+gamma_b/2*(exp(x)^2)));
...
r_ann=(r_bar)^4;
r_star_ann=(r_star_bar)^4;
pi_ann=0;
pih_ann=0;
ca=0;
spr_star=(rk_star_bar^4-r_star_bar^4);
exp_spr_star=(rk_star_bar^4-r_star_bar^4);
nw_mkt_star=nw_star;
lev_mkt_star=lev_star;
b_star_h=b_star;
eqp_est=q*100;
realGDP_est=y*100;
realexr_est=rer*100;
...
exp_spr_star_est=(rk_star_bar^4-r_star_bar^4)*10000;
r_ann_est=((r_bar)^4)*100;
pi_ann_est=0;
pi_star_ann_est=0;